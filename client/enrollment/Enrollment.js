...
// Pg 272, the enrollment component
export default function Enrollment ({match}) {
  ...
  // Pg 279, number of completed lessons
  ...
  // Pg 274, when a drawer item is clicked, we will call the select drawer method
  const selectDrawer = (index) => event => {
      setValues({...values, drawer:index})
  }
  // Pg 278, marks lesson complete
  const markComplete = () => {
    ...
    const imageUrl = enrollment.course._id
     ...
     // Pg 272, the enrollment component
     // Pg 274, actual content is rendered depending on the drawer value
     // Pg 277, completed lessons from the view
          return (
