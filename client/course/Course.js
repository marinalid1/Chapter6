...
export default function Course ({match}) {
  ...
  // Pg 284, displaying enrollment stats for a published course
  useEffect(() => {
    const abortController = new AbortController()
    const signal = abortController.signal
  ...
  // Pg 245, addLesson function is executed to render the latest lesson
  const addLesson = (course) => {
    setCourse(course)
  }

  // Pg 258, when the PUBLISH button is clicked, we will open a dialog asking the user for confirmation
  const clickPublish = () => {
  ...
  // Pg 259, when publish is confirmed, we will make an update API call to the backend
  const publish = () => {
  ...
  if (values.redirect) {
    ...
    const imageUrl = course._id
          ...
          // Pg 237, render the recieved details, such as course name, description, category, image and a link to the instructors user profile
          // Pg 245, the NewLesson compoenent should only render if the current user is the instructor of the course
          // Pg 246, render the list of lessons
          // Pg 256, the delete course component
          // Pg 258, Publish button states
          // Pg 259, Confirm to publish
          // Pg 284, displaying enrollment stats for a published course
          return (
